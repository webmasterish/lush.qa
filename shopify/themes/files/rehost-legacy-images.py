...
def digests(product_ids):
    ...
    Read *after* the English description is written, never before: updating
    descriptionHtml changes the source, which changes the digest, and a stale
    one fails with "Translatable content hash is invalid".
    """
    out = {}
...
def existing_files():
    """Everything already in Files, indexed by normalised filename.

    Makes the run idempotent and, more to the point, survivable: once the
    English descriptions are rewritten the live twins disappear from the scan,
    so a dead Arabic URL can only be resolved against what was uploaded on an
    earlier pass. `alt` carries the original filename for exactly this.
    """
    index, cursor = {}, None
    while True:
        data = gql("""query($c:String){ files(first:250, after:$c, query:"media_type:IMAGE"){
            pageInfo{ hasNextPage endCursor }
            nodes{ ... on MediaImage { alt image{ url } } } } }""", {'c': cursor})['files']
        for n in data['nodes']:
            url = (n.get('image') or {}).get('url')
            if not url:
                continue
            for name in (n.get('alt'), url.split('?')[0]):
                if name:
                    index.setdefault(basename(name), url)
                    index.setdefault(basename(name, True), url)
        if not data['pageInfo']['hasNextPage']:
            break
        cursor = data['pageInfo']['endCursor']
    return index
...
def main():
    ...
    for u in dead:
        ...
        if match:
            twin[u] = match
            continue
        hosted = already.get(basename(u)) or already.get(basename(u, True))
        if hosted:
            from_files[u] = hosted
    unresolved = sorted(dead - set(twin) - set(from_files))
    ...
    print(f'  already 404, but the photo is already in Files {len(from_files)}')
    ...
    MANIFEST.write_text(json.dumps({
        ...
        'dead_resolved_from_files': {k: v for k, v in sorted(from_files.items())},
    ...
    replacements.update(from_files)
    ...
    for p in products:
        ...
        if p['ar']:
            ...
            # Re-read the digest here, after the English write above.
            digest = digests([p['id']]).get(p['id']) if new_ar != p['ar'] else None
