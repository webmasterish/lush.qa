...
| DotAim admin account, also `dev@dotaim.com` | Settings > Users, per `store-settings-ledger.md` | Last, after everything else is confirmed. Same address as the notification recipient above, so removing one does not remove the other. **2026-09-09: this may now be forced early.** The Grow plan's five user seats are full, so adding Sibin means freeing one, and removing this account is the first of the two options put to Dee. If she takes it, the revocation happens on her timetable rather than ours, and we keep working through the `shopify.partner@dotaim.com` collaborator account, which consumes no seat but cannot reach Settings > Users |
