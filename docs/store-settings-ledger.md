...
| URL redirects | Settings > Navigation > URL redirects | **0 on the store.** 471 are built and ready in `shopify/themes/redirects/redirects.json`; apply with `build-redirects.py --apply`. Two decisions first: whether dead product URLs 404 or fall back to a collection, and whether an About page exists to receive `/about` | yes | todo | DotAim |
